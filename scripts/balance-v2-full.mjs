// Historical global cost/ROI route policy; uses a named immutable runtime snapshot.
...
export function simulate(route, { afterIncome = null, seed = 17, limit = maxSeconds, stopAtNether = false } = {}) {
  ...
  for (; time < limit && !s.completed && !(stopAtNether && n(s,'N1')); time++) {
    ...
    if(!s.research.completed.industrial && n(s,'V11') && n(s,'V4')===1 && s.money>=price(s,ITEMS.V4)){if(purchase(ITEMS.V4))continue;}
