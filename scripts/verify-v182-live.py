...
R=Path('/Users/kevinyoung/vibe-coding/my-profile/projects/mc-clicker-2');out=R/'docs/v1.8/qa/live-1.8.2/browser';out.mkdir(parents=True,exist_ok=True)
...
with sync_playwright() as p:
 for engine,width in [('chromium',1440),('webkit',390),('chromium',320)]:
  ...
  assert 'V1.8.2' in page.title();assert page.evaluate('typeof mcDebug')=='undefined'
  ...
  click('#info-open');click('#info-tab-versions');assert 'V1.8.2' in page.locator('#info-versions').inner_text()
  assert not errors,errors
  reports.append({'engine':engine,'width':width,'version':'1.8.2','noDebug':True,'mailStoreCancelReload':True,'freePlacementEntry':True,'threeBasicHomes':True,'versionHistory':True,'noOverflow':True,'errors':errors});browser.close()
