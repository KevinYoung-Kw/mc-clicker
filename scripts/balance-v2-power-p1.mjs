// Power policy P1: chapter policy with demand-led electricity investment only.
...
export function simulate(route, { afterIncome = null, seed = 17, limit = maxSeconds, stopAtNether = false } = {}) {
  ...
  function planPower(next) {
    ...
    if (next && !generationIds.has(next.id) && s.money >= price(s, next)) {
      const copy = structuredClone(s);
      if (buy(copy, next.id).ok) {
        connectAll(copy);
        const planned = powerSnapshot(copy).demand;
        if (planned > demand) { demand = planned; reason = `planned:${next.id}`; }
      }
    }
    if (current.supply >= demand - 0.01 || demand <= 0) return false;
    const target = demand * 1.2, candidates = [];
    ...
    const choice = covers.sort((a,b) => a.cost - b.cost)[0] || candidates.sort((a,b) => a.cost / (a.supply-current.supply) - b.cost / (b.supply-current.supply))[0];
