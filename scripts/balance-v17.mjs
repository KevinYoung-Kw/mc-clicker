...
import { connectAll, setAutoConnect, powerSnapshot } from '../src/power.js';
import { buyGuidance } from '../src/guidance.js';
import { createHash } from 'node:crypto';
import { pathToFileURL } from 'node:url';
const sourceHashes = Object.fromEntries(['game','operations','power','dimensional','facility-capacity','upgrade-catalog'].map(n=>[n,createHash('sha256').update(readFileSync(new URL('../src/'+n+'.js',import.meta.url))).digest('hex')]));
...
const args = process.argv.slice(2),
  option = (key, fallback) =>
    args[args.indexOf(key) + 1] && args.includes(key)
      ...
      : fallback;
const out = option("--out", "docs/v1.7/qa/balance-current");
const maxSeconds = +option("--max-seconds", "10800");
const selected = option("--route", "all");
mkdirSync(out, { recursive: true });

export function simulate(route, { afterIncome = null, seed = 17, limit = maxSeconds } = {}) {
  const s = fresh(0),
  ...
  for (; time < limit && !s.completed; time++) {
    const total = s.total;
    advance(s, 1);
    if (time % (n(s, "V2") ? route.click : 1) === 0) mine(s, () => 1);
    actualIncome += s.total - total;
    if(n(s,'M5')&&time%15===0){connectAll(s);if(s.grid.learnedConnection&&!s.grid.autoConnect)setAutoConnect(s,true);}
    ...
    if (n(s,'V2') && (starterHomeAvailable(s) || housingCapacity(s)<Math.min(route.residents,n(s,'V2')+1))) { if(house())continue; }
    if (
...
for (const route of ROUTES.filter(
  ...
  const r = simulate(route, { afterIncome: trace?.tick }),
    { state, ...report } = r;
  writeFileSync(
    `${out}/${route.id}.json`,
...
writeFileSync(
  ...
  JSON.stringify(
    {
      method:
        "Current engine; real prices, paid land/housing, free starter home, level prerequisites, explicit connections, jobs and upgrades. Foreground 1s steps, scripted route-specific tap frequency; no reward mail or money injection into the played state. ROI probes use cloned states only. Not a human playthrough or a measured human completion time.",
