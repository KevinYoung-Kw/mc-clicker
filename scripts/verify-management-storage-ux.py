...
R=Path(__file__).resolve().parents[1];out=R/'docs/v1.8/qa/management-storage-ux';out.mkdir(parents=True,exist_ok=True)
...
with sync_playwright() as p:
 for engine,width in [('chromium',1440),('webkit',390),('chromium',320)]:
  ...
  try:
   page.goto('http://127.0.0.1:8932/',wait_until='networkidle');pause();click('[data-nav="network"]');shot('industry')
   ...
   before=page.evaluate('({energy:mcDebug.state.energy,placement:mcDebug.state.placements.M5})')
   ...
   assert page.evaluate('({energy:mcDebug.state.energy,placement:mcDebug.state.placements.M5})')==before
