// P8: P7 plus actual role quotas; no avoidable idle residents or missing hosts.
...
export function simulate(route, { afterIncome = null, seed = 17, limit = maxSeconds, stopAtNether = false } = {}) {
  ...
  return {
    provenance:{engine:engineName,engineSha256:sourceHashes.sha256,policy:lifePolicy?'P8-staffed-life':'P8-staffed',policySha256:createHash('sha256').update(readFileSync(new URL(import.meta.url))).digest('hex'),seed,stopAtNether},
    foodCare,
    electricity,
...
writeFileSync(
  ...
  JSON.stringify(
    {
      method:
        "Frozen named runtime; sourceHashes identifies exact parameters. Real purchases, paid land/housing, jobs, production and settlement; no reward mail or injected assets. Foreground 1s steps, route-specific tapping. Power responds to current load or an affordable planned build. Optional P6 food-response policy reserves basic meal service before grace ends; preference is declared, not inferred from ROI. P8 assigns real available role quotas via assignJob, preserving cargo restrictions and filling new host roles. It also considers affordable first purchases outside the route list; personal skill candidates skip capped levels. Optional life-policy enables tea only on village-life with five minutes budget. General ROI still uses nominal rates, not realised return. Successful management/work intervals exclude narration and repeated mining; they are NOT all available opportunities or proof of fun. Research-only wait is unmeasured (null), even when mainline research is instant. actualIncome includes manual taps; do not add income.manual again. Not a human playthrough.",
