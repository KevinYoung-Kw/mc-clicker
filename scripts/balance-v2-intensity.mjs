...
export function simulate(profile,{duration=20,gates='strong',seconds=7200,seed=17,maintenance=true,adaptive=false}={}) {
  ...
  function resolveResearch(id,seen) {
    ...
    if(gates==='strong'&&id==='industrial'){
      if((s.community.residents||[]).filter(r=>!r.reserve).length<(row.residents||4))return 'V2';
      if(HD.housingCapacity(s)<(row.residents||4))return 'housing';
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){
  ...
  const out=new URL(`../docs/v2.0.0/simulation/${arg('--out','intensity-routes.json')}`,import.meta.url);
