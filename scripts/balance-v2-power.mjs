// Power policy P5: retain the actual planned build while paying for its power.
...
const {game,catalog,upgrades,residents,housing,housingData,power,guidance,research,food}=await import(`../docs/v2.0.0/simulation/${engineName}.mjs`);
...
export function simulate(route, { afterIncome = null, seed = 17, limit = maxSeconds, stopAtNether = false } = {}) {
  ...
  return {
    provenance:{engine:engineName,engineSha256:sourceHashes.sha256,policy:respondsToFood?'P6-food-response':'P5',policySha256:createHash('sha256').update(readFileSync(new URL(import.meta.url))).digest('hex'),seed,stopAtNether},
    foodCare,
    electricity,
