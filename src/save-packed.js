...
// Format 1 is frozen: never regenerate its defaults when game defaults change.
// Known fields/values live in the game; unknown values remain in the code.
// Reference and difference records retain historical snapshots exactly.
...
export function packSave(value) {
  ...
  function remember(v, key) {
    ...
    if (!Array.isArray(v)) {
      const shape = JSON.stringify(Object.keys(v)), candidates = similar.get(shape) || [];
      candidates.push(id); if (candidates.length > 12) candidates.shift(); similar.set(shape, candidates);
    }
  }
  ...
  function write(v, depth = 0) {
    ...
    } else if (typeof v === 'string') { byte(6); text(v); }
    else if (Array.isArray(v)) { byte(7); uint(v.length); for (const x of v) write(x, depth + 1); }
    else {
      const keys = Object.keys(v), shape = JSON.stringify(keys);
      let base = -1, changed, least = keys.length;
      if (keys.length >= 3 && key.length >= 64) for (const id of similar.get(shape) || []) {
        const flags = keys.map(k => JSON.stringify(v[k]) !== JSON.stringify(memo[id][k]));
...
export function unpackSave(bytes) {
  ...
  function read(depth = 0) {
    ...
    if (tag === 0) v = null;
      ...
      const keys = Object.keys(base.value), masks = Array.from({ length: Math.ceil(keys.length / 8) }, byte);
      ...
      const index = uint(); let keys;
      if (index) { keys = shapes[index - 1]; if (!keys) throw Error('存档结构损坏。'); }
      else {
        const count = uint(); if (count > LIMIT) throw Error('存档字段过多。'); keys = [];
        for (let i = 0; i < count; i++) keys.push(text());
        if (new Set(keys).size !== keys.length || keys.some(forbidden)) throw Error('存档字段无效。'); shapes.push(keys);
      }
      charge(2 + keys.length * 2); v = {};
