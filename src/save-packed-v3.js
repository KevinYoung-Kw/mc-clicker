...
import transcripts from './save-format/transcripts-v1.json' with { type: 'json' };
// Frozen structure 3: structure 2 tags plus source-authored transcript records.
// Table order is part of the wire format. Never import live narrator copy here.
// Unmatched historical/custom text is written literally, with exact ordering.
...
for (const record of transcripts) seed(record);
const trie = new Map();
...
export function packSaveV3(value) {
...
export function unpackSaveV3(bytes) {
