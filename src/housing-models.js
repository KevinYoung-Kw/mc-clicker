// Accepted eight-house art study, now used by the playable village.
import * as T from 'three';
import {blockBox,cube,group,mat} from './models.js';
import {HOMES,HOME_BY_ID} from './housing-data.js';
export {HOMES};
import {cottage} from './village-models.js';
...
function gable(g,x,z,w,d,y,rise,c,axis='x',n=8){
 ...
 for(let i=0;i<n;i++){
  ...
  for(const s of [-1,1])B(r,'wood',c,s*(width/2-step/4),i*rise/n,0,step/2+.032,rise/n,d);
  for(const s of [-1,1]) B(r,'wood',plaster,0,i*rise/n-.014,s*(d/2-.12),Math.max(.065,width-.16),rise/n,.055);
 }
...
function pane(g,x,y,z,{turn=0,w=.26,h=.28,night=false,shutters=false,cross=true}={}){
 ...
 m.material=mat(night?'#ffcb79':'#739f9b',night?.85:0);m.userData.windowGlow=true;
 if(cross){
  B(r,'wood','#cbb990',0,0,.055,.026,h,.025);
  for(const side of [-1,1])B(r,'wood','#cbb990',side*(w+.026)/4,0,.055,(w-.026)/2,.026,.025);
 }
...
function stairs(g,x,z,y,w,run,rise,n=6){
 for(let i=0;i<n;i++)B(g,'cobblestone',stone,x,y+(i+1)*rise/n/2,z-(i+.5)*run/n,w,(i+1)*rise/n,run/n);
}
...
function houseMoss(g,night){
 ...
 B(g,'wood','#747553',0,.69,.49,.63,.13,.23);
...
function houseCorner(g,night){
 ...
 door(g,-.66,.232,1.04,night,{h:.54,number:3});pane(g,-.15,1.55,.232,{night,w:.24,h:.26});
 pane(g,-1.079,.64,-.31,{night,turn:-Math.PI/2,w:.37,h:.30});
 stairs(g,.86,.81,.1,.40,1.45,1.04,8);B(g,'stone',stone,.75,1.15,-.69,.61,.075,.33);
...
export function homeModel(spec,night=false){
 ...
 if(spec.id==='cottage'){cottage(g,0).scale.setScalar(.7);return g;}
...
export function homePreview(type,rotation=0,variant=0){
 const root=['legacy','cottage'].includes(type)?new T.Group():homeModel(HOME_BY_ID[type]);if(['legacy','cottage'].includes(type))cottage(root,variant).scale.setScalar(.7);root.position.y=.16;root.rotation.y=rotation*Math.PI/2;
 const materials=new Map();root.traverse(o=>{if(!o.isMesh)return;const source=o.material;if(!materials.has(source)){const m=source.clone();m.transparent=true;m.opacity=.48;m.depthWrite=false;m.userData.original=m.color.clone();materials.set(source,m);}o.material=materials.get(source);o.castShadow=false;o.receiveShadow=false;o.renderOrder=11;});
 return {root,setValid(valid){for(const m of materials.values())m.color.copy(m.userData.original).lerp(new T.Color('#be6b58'),valid?0:.65);},dispose(){root.removeFromParent();for(const m of materials.values())m.dispose();}};
}
